...
document.getElementById("submit-query").addEventListener("click", function() {
    ...
    var correctQuery = "SELECT UPPER(FabrickFloorName) AS FabrickFloorName FROM QueryFactory WHERE FabrickFloorName = 'distinct';";
...
function recordLevelCompletion(playerId, elapsedTime) {
    fetch('/record_level_completion/', {
        ...
        body: JSON.stringify({
            'player_id': playerId,
            'level_id': 13,  // Replace with the actual level ID
