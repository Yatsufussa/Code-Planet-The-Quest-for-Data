...
document.getElementById("submit-query").addEventListener("click", function() {
    ...
    var correctQuery = "SELECT CONCAT_WS('', (SELECT WayofTheMountain FROM game_app_mountain_of_algorithms WHERE WayOfTheMountain = 'Quick'), (SELECT MountainKeeper FROM game_app_Mountain_Of_Algorithms WHERE MountainKeeper = 'Sort')) AS MountainTechnique;";
...
function fetchHint() {
    return fetch('/get_hint/?level_id=3')
        .then(response => response.json())
        .then(data => data.hint)
        .catch(error => {
            console.error('Error fetching hint:', error);
...
function recordLevelCompletion(playerId, elapsedTime) {
    fetch('/record_level_completion/', {
        ...
        body: JSON.stringify({
            'player_id': playerId,
            'level_id': 3,  // Replace with the actual level ID
