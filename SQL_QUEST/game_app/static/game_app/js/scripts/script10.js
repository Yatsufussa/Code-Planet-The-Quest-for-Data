...
document.getElementById("submit-query").addEventListener("click", function() {
    ...
    var correctQuery = "SELECT REPLACE(AccessKeyValue, 'R SA', 'RSA') AS CorrectedValue FROM Lunar_Landscape WHERE AccessKeyValue LIKE '%R SA%';";
...
function fetchHint() {
    return fetch('/get_hint/?level_id=10')
        .then(response => response.json())
        .then(data => data.hint)
        .catch(error => {
            console.error('Error fetching hint:', error);
