...
document.getElementById("submit-query").addEventListener("click", function() {
    ...
    var correctQuery = "SELECT REPLACE(IndexType, 'text-Full', 'Full-text') AS ModifiedValue FROM Optimization_Plateau WHERE IndexType = 'text-Full';";
...
function fetchHint() {
    return fetch('/get_hint/?level_id=9')
        .then(response => response.json())
        .then(data => data.hint)
        .catch(error => {
            console.error('Error fetching hint:', error);
...
function recordLevelCompletion(playerId, elapsedTime) {
    fetch('/record_level_completion/', {
        ...
        body: JSON.stringify({
            'player_id': playerId,
            'level_id': 9,  // Replace with the actual level ID
