...
document.getElementById("submit-query").addEventListener("click", function() {
    ...
    var correctQuery = "SELECT REPLACE(VirusFound, 'WHORO', 'WHERE') AS Answer FROM DatabaseDepths2 WHERE VirusFound = 'WHORO';";
...
function recordLevelCompletion(playerId, elapsedTime) {
    fetch('/record_level_completion/', {
        ...
        body: JSON.stringify({
            'player_id': playerId,
            'level_id': 17,  // Replace with the actual level ID
