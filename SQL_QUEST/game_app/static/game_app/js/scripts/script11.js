...
document.getElementById("submit-query").addEventListener("click", function() {
    ...
    var correctQuery = "SELECT SUBSTRING(TunnelName, 5, 4) AS ExtractedSubstring FROM PlanetDungeons WHERE SUBSTRING(TunnelName, 5, 4) = 'JOIN';";
...
function recordLevelCompletion(playerId, elapsedTime) {
    fetch('/record_level_completion/', {
        ...
        body: JSON.stringify({
            'player_id': playerId,
            'level_id': 11,  // Replace with the actual level ID
