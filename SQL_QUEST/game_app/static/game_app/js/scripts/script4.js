...
document.getElementById("submit-query").addEventListener("click", function() {
    ...
    var correctQuery = "SELECT TRIM(Actions) AS CleanedAction FROM game_app_Log_Desrt WHERE TRIM(Actions) = 'Injection';";
...
function fetchHint() {
    return fetch('/get_hint/?level_id=4')
        .then(response => response.json())
        .then(data => data.hint)
        .catch(error => {
            console.error('Error fetching hint:', error);
...
function recordLevelCompletion(playerId, elapsedTime) {
    fetch('/record_level_completion/', {
        ...
        body: JSON.stringify({
            'player_id': playerId,
            'level_id': 4,  // Replace with the actual level ID
