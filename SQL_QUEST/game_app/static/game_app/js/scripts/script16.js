...
document.getElementById("submit-query").addEventListener("click", function() {
    ...
    var correctQuery = "SELECT SUBSTRING(CloudName, 1, 3) AS ExtractedValue FROM ProcessingClouds2 WHERE CloudName = 'AVGCloud';";
...
function recordLevelCompletion(playerId, elapsedTime) {
    fetch('/record_level_completion/', {
        ...
        body: JSON.stringify({
            'player_id': playerId,
            'level_id': 16,  // Replace with the actual level ID
