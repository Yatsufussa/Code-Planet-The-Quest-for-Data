...
document.getElementById("submit-query").addEventListener("click", function() {
    ...
    var correctQuery = "SELECT CONCAT((SELECT SUBSTRING(SafeCode, 1, 1) FROM lvl19_2 WHERE SafeCode = 'I124123'),(SELECT REVERSE(SafeCode) FROM lvl19_2 WHERE SafeCode = 'TRESN')) AS Result LIMIT 1;";
...
function recordLevelCompletion(playerId, elapsedTime) {
    fetch('/record_level_completion/', {
        ...
        body: JSON.stringify({
            'player_id': playerId,
            'level_id': 19,  // Replace with the actual level ID
